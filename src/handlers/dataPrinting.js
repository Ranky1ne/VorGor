...
export const dataPrinting = async (request, response) => {
    ...
    const result =  await query(connection,
        ...
        console.log(car)
        await query(connection,
            "INSERT INTO `journal`( `Номер Заявки`,`Номер ТТН`, `Заказчик`, `Перевозчик`, `Объем`, `Марка машины`, `Номер машины`, `Сырье`) VALUES (? ,? ,? ,? ,? ,? ,? ,? )",
            [orderId, numberTTN, car[0][`Заказчик`], car[0][`Перевозчик`], car[0][`Объем, м3`], car[0][`Марка машины`], car[0][`Номер машины`], car[0][`Сырье`],]
        )
        response.end()
}